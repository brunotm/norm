...
var (
	selectCases = []struct {
	...
	}{
		{
			...
			stmt: Select("id", "user", "email", "role").From("users").Where("email = ?", "john.doe@email.com").
		...
		},
		{
			...
			stmt: Select("*").With("select_offices",
				Select("country", "city", "address", "postal_code").From("offices").WhereIn("country", "uk", "es", "pt", "fr")).
				From("users").JoinInner("select_offices", "users.city = select_offices.city").
		...
		},
		{
			...
			stmt: Select("sub_part", "SUM(quantity) as total_quantity").From("included_parts").
				GroupBy("sub_part").
				WithRecursive(
					"included_parts",
